...
public class Test_1028_102801 {

    @Rule
    public ActivityTestRule<MainActivity> activityRule = new ActivityTestRule<>(MainActivity.class);

    @Test
    public void testEnabledisableToggleSwitches() throws InterruptedException {
    ...
    private void verifyState(int index, boolean expectedState) {
        switch (index) {
            case 0:
                onView(withId(R.id.id_settings_severe_weather_alerts_switch))
                        .check(matches(expectedState ? isChecked() : isNotChecked()));
                break;
            case 1:
                onView(withId(R.id.id_settings_station_alerts_switch))
                        .check(matches(expectedState ? isChecked() : isNotChecked()));
